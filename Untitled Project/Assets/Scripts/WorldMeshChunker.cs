    ...
    void Update()
    {
        if (run)
        {
            ...
            for (int i = -64; i <= 64; i++)
            {
                ...
                var (positiveVertices_V, positiveTriangles_V, positiveUVs_V, positiveUV2s_V, positiveColors_V, negativeVertices_V, negativeTriangles_V, negativeUVs_V, negativeUV2s_V, negativeColors_V) =
                    this.GetComponent<Sliceable>().LineSliceMesh(vertices_V, triangles_V, UVs_V, UV2s_V, colors_V, new Vector3(1.0f, 0.0f, 0.0f), planePosition_V, -1000000.0f, 1000000.0f);
                ...
                for (int j = -64; j <= 64; j++)
                {
                    ...
                    var (positiveVertices_H, positiveTriangles_H, positiveUVs_H, positiveUV2s_H, positiveColors_H, negativeVertices_H, negativeTriangles_H, negativeUVs_H, negativeUV2s_H, negativeColors_H) =
                        this.GetComponent<Sliceable>().LineSliceMesh(vertices_H, triangles_H, UVs_H, UV2s_H, colors_H, new Vector3(0.0f, 1.0f, 0.0f), planePosition_H, -1000000.0f, 1000000.0f);
